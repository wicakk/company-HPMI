...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
      <p class="text-xs font-bold uppercase tracking-widest text-violet-500 dark:text-violet-400 mb-1">Perpustakaan</p>
  ...
  <form method="GET" class="flex flex-col sm:flex-row gap-3">
    <div class="relative flex-1">
      ...
      <input type="text" name="q" value="{{ request('q') }}"
             ...
             class="w-full pl-10 pr-4 h-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 transition">
    </div>

    @if($categories->count())
    <select name="category"
      class="h-10 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500 cursor-pointer">
    ...
    <button type="submit"
      class="h-10 px-5 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold rounded-xl transition active:scale-95 flex items-center gap-2">
  ...
  <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
    ...
    @php
      ...
      $canDownload = !$isPremium || auth()->user()->isPremium();
      $ext         = strtoupper($journal->file_type ?? pathinfo($journal->file_path ?? '', PATHINFO_EXTENSION) ?? 'PDF');
    @endphp
    ...
    <div class="flex items-start gap-4 px-6 py-5 border-b border-slate-50 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition group">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          @if($journal->category)
          <span class="text-slate-300 dark:text-slate-600">·</span>
          <span class="px-2 py-0.5 bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 text-[10px] font-semibold rounded-lg">
            {{ $journal->category }}
          </span>
          @endif
      ...
      <div class="flex-shrink-0 flex flex-col items-end gap-2">
        ...
        <a href="{{ route('member.journals.download', $journal) }}"
           class="inline-flex items-center gap-1.5 px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold rounded-xl transition active:scale-95 shadow-sm shadow-violet-500/20">
    ...
    <div class="py-20 text-center">
      <div class="w-20 h-20 bg-violet-50 dark:bg-violet-900/20 rounded-3xl flex items-center justify-center mx-auto mb-4">
      ...
      @if(request()->hasAny(['q','category']))
      <a href="{{ route('member.journals') }}"
         class="inline-flex items-center gap-1.5 mt-4 px-4 py-2 bg-violet-600 text-white rounded-xl text-xs font-semibold hover:bg-violet-700 transition">
        Reset Pencarian
      </a>
      @endif
    </div>
  ...
  {{-- Banner upgrade jika bukan premium --}}
  @if(!auth()->user()->isPremium())
  <div class="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-5">
    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-amber-700 dark:text-amber-400 text-xs mt-1">Upgrade ke Premium untuk mengakses semua jurnal eksklusif HPMI.</p>
      </div>
      <a href="{{ route('member.payment') }}"
