...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ openModal: false }">
  ...
  {{-- ══ STATE 3: ADA TAGIHAN, BELUM KONFIRMASI ══ --}}
  ...
  <div class="relative bg-gradient-to-br from-slate-900 via-primary-900 to-violet-900 rounded-3xl p-8 text-white mb-5 overflow-hidden shadow-2xl">
    ...
    <div class="relative">
      ...
      {{--
        SOLUSI quote conflict:
        Nilai PHP disimpan ke atribut data-va di HTML
        Alpine membacanya via $el.dataset.va — tidak ada PHP string di dalam JS string
      --}}
      <div class="flex items-center gap-3 mb-6"
           x-data="{ copied: false }"
           x-on:click="navigator.clipboard.writeText($el.dataset.va); copied = true; setTimeout(() => copied = false, 2000)"
           data-va="{{ $pendingVa }}">
        <p class="font-mono text-2xl font-black tracking-widest cursor-pointer hover:text-primary-200 transition">
          {{ $pendingVa }}
        </p>
        <button type="button" class="bg-white/10 hover:bg-white/20 rounded-xl px-3 py-1.5 text-xs font-semibold transition">
    ...
    <div class="divide-y divide-slate-100 dark:divide-slate-800">
      ...
      <div class="px-6 py-4 flex items-center gap-4" x-data="{ copied: false }">
        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center flex-shrink-0 font-black text-slate-600 dark:text-slate-300 text-xs overflow-hidden">
          @if($bLogo)
            <img src="{{ Storage::url($bLogo) }}" alt="{{ $bName }}" class="w-full h-full object-contain p-1">
          @else
        ...
        {{-- data-no menghindari quote conflict --}}
        <button type="button"
                data-no="{{ $bNo }}"
  ...
  <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 mb-5">
    ...
    <ol class="space-y-3">
      <li class="flex items-start gap-3">
        <span class="w-6 h-6 rounded-full bg-rose-500 text-white text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
        <p class="text-sm text-slate-600 dark:text-slate-400">Pilih rekening tujuan di atas, lalu salin nomor rekeningnya.</p>
      </li>
      <li class="flex items-start gap-3">
        <span class="w-6 h-6 rounded-full bg-orange-500 text-white text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
        <p class="text-sm text-slate-600 dark:text-slate-400">Transfer sejumlah <strong>Rp {{ number_format($pendingAmount, 0, ',', '.') }}</strong> dari bank atau e-wallet Anda.</p>
      </li>
      <li class="flex items-start gap-3">
        <span class="w-6 h-6 rounded-full bg-amber-500 text-white text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
        <p class="text-sm text-slate-600 dark:text-slate-400">Simpan bukti transfer (screenshot/foto struk).</p>
      </li>
      <li class="flex items-start gap-3">
        <span class="w-6 h-6 rounded-full bg-emerald-500 text-white text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">4</span>
        <p class="text-sm text-slate-600 dark:text-slate-400">Klik tombol <strong>"Konfirmasi Pembayaran"</strong> di bawah dan isi form konfirmasi.</p>
      </li>
      <li class="flex items-start gap-3">
        <span class="w-6 h-6 rounded-full bg-blue-500 text-white text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">5</span>
        <p class="text-sm text-slate-600 dark:text-slate-400">Admin akan memvalidasi dalam <strong>1×24 jam kerja</strong>. Status diperbarui otomatis.</p>
      </li>
    </ol>
  </div>
...
<div x-show="openModal"
  ...
  <div class="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto"
    ...
    <form method="POST"
          action="{{ route('member.payment.confirm', $pendingId) }}"
      ...
      {{-- Nominal readonly --}}
      <div>
        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wide mb-1.5">Nominal Transfer</label>
      ...
      <div>
        ...
        <input type="text" name="sender_name" value="{{ old('sender_name') }}"
               ...
               class="w-full px-4 py-2.5 text-sm border rounded-xl bg-slate-50 dark:bg-slate-700/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition {{ $errors->has('sender_name') ? 'border-red-400' : 'border-slate-200 dark:border-slate-600' }}">
      ...
      <div>
        ...
        <select name="payment_method" required
                class="w-full px-4 py-2.5 text-sm border rounded-xl bg-slate-50 dark:bg-slate-700/50 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition {{ $errors->has('payment_method') ? 'border-red-400' : 'border-slate-200 dark:border-slate-600' }}">
          <option value="">Pilih bank/e-wallet yang digunakan...</option>
          @foreach($banks as $bank)
          @php $bName = data_get($bank, 'bank_name', ''); $bNo = data_get($bank, 'account_number', ''); @endphp
          <option value="{{ $bName }}" {{ old('payment_method') == $bName ? 'selected' : '' }}>
            {{ $bName }} — {{ $bNo }}
          </option>
          @endforeach
      ...
      <div>
        ...
        <input type="date" name="transfer_date"
               ...
               class="w-full px-4 py-2.5 text-sm border rounded-xl bg-slate-50 dark:bg-slate-700/50 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition {{ $errors->has('transfer_date') ? 'border-red-400' : 'border-slate-200 dark:border-slate-600' }}">
      ...
      <div>
        ...
        <label class="block cursor-pointer" x-data="{ fileName: '' }">
          <div class="border-2 border-dashed rounded-xl p-5 text-center transition"
               :class="fileName ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20' : 'border-slate-200 dark:border-slate-600 hover:border-emerald-400'">
      ...
      <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 text-xs text-amber-700 dark:text-amber-400 flex gap-2">
        ...
        <p>Pastikan data sesuai bukti transfer. Admin akan memvalidasi dalam <strong>1×24 jam kerja</strong>.</p>
      </div>
