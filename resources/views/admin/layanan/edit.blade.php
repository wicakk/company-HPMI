...
<div class="px-6 py-6 max-w-3xl">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.layanan.update', $layanan) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="kategori"
                        ...
                        @foreach($kategoriOptions as $k)
                            <option value="{{ $k }}" @selected(old('kategori', $layanan->kategori) === $k)>{{ $k }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="urutan" value="{{ old('urutan', $layanan->urutan) }}" min="0"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <textarea name="deskripsi_singkat" rows="3" maxlength="500"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none @error('deskripsi_singkat') border-red-400 @enderror">{{ old('deskripsi_singkat', $layanan->deskripsi_singkat) }}</textarea>
            ...
            <div>
                ...
                <textarea name="deskripsi_lengkap" rows="6"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('deskripsi_lengkap', $layanan->deskripsi_lengkap) }}</textarea>
            ...
            <div>
                ...
                @if($layanan->gambar_url)
                    <div class="mb-3">
                        <img src="{{ $layanan->gambar_url }}" alt="Gambar saat ini"
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="aktif"
                               @checked(old('status', $layanan->status) === 'aktif') class="accent-blue-600">
                        <span class="text-sm text-gray-700">Aktif</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="nonaktif"
                               @checked(old('status', $layanan->status) === 'nonaktif') class="accent-blue-600">
